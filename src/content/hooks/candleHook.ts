...
const useCandleHook = function (
	...
	const initStaticData = function () {
		...
		//用于显示的数据
		let result = findDataByTicks(xAxis.data.tickArr) as { data: IcandleData[]; scope: numberScopeString };
	...
	const updatePartialCandleData = function () {
		...
		//用于显示的数据
		let result = findDataByTicks(_xAxisdatatickArr) as { data: IcandleData[]; scope: numberScopeString };
	...
	*/
	const updatePartialCandleDataV3 = function () {
		...
		let updateAndAppendNewCandle = function () {
			...
			let _displayCandleData = [...backwardDCArr, ..._totalCandleDisplayArr, ...forwardDCArr].sort(function (
				a,
				b
			) {
				return Number(a.time) - Number(b.time);
		...
		window.requestAnimationFrame(function () {
			if (xAxis.data.tickArr.length > 120) {
				...
				if (
					xAxis.data.lastTimeScope.start !== xAxis.data.currentTimeScope.start &&
