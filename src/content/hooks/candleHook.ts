...
import { anyTimeToGMT0000ToTarget, findIntersectionCandle, getRightDate, getSpaceSize, newGuid } from "../utils/consts";
 ...
 */
const useCandleHook = function (args: IdataConfig, xAxis: IAxisobj, yAxis: IyAxisobj, baseConfig: IuseCandleView): IuseCandleHook {
	xAxis = xAxis!;
	yAxis = yAxis!;
	const [initArgs, setinitArgs] = useState<IdataConfig>(lodash.merge(DEFAULTDATAVALUES, args));
	...
	const [stopDynamicFetching, setstopDynamicFetching] = useState<boolean>(false);
	 ...
	 * 动态数据暂存处
	 */
	const [TempDynamicData, setTempDynamicData] = useState<IcandleItem[] | null>(null);
	...
	const [cursorCandleItem, setCursorCandleItem] = useState<IcandleData | null>(null);
	const [latestCandleItem, setlatestCandleItem] = useState<IcandleData | null>(null);
	 ...
	 * 记录当前视窗内y轴数据的最大值和最小值
	 */
	const [displayCandleMaxMin, setdisplayCandleMaxMin] = useState<numberScopeString>({ start: "0", end: "0" });
	const [org_displayCandleMaxMin, setorg_displayCandleMaxMin] = useState<numberScopeString>({ start: "0", end: "0" });
	...
	const [latestCandleToolTip, setlatestCandleToolTip] = useState<IToolTipItem | null>(null);
	const [latestVolumeToolTip, setlatestVolumeToolTip] = useState<IToolTipItem | null>(null);

	/**
	 * 视图范围内最末尾的candle
	 * 最新的candle
	 */
	const [displayLatestCandle, setdisplayLatestCandle] = useState<IcandleData | null>(null);
	const [latestdisplayLatestCandle, setlatestdisplayLatestCandle] = useState<IToolTipItem | null>(null);
	const [latestdisplayLatestVolume, setlatestdisplayLatestVolume] = useState<IToolTipItem | null>(null);
	 ...
	 */
	let allComputedCandleData = useRef<{ [propName: string]: IcandleItem }>({} as { [propName: string]: IcandleItem });
	 ...
	 */
	const determineTimeSpaceConsistent = function (data: IcandleItem[]): "same" | "bigger" | "smaller" {
		if (data.length === 1) {
			return "smaller";
		}
	...
	*/
	 ...
	 * @param data 输入数据
	 */
	const putDataIntoAllComputedCandleData = function (data: IcandleItem[]): numberScopeString {
		let _displayCandleMaxMin: numberScopeString = {
			/**
			 * 最低点
			 * */
	...
	const getCandleColor = function (start: string, end: string, type: "wick" | "candle"): string {
	...
	const getCandleStatus = function (start: string, end: string, type: "wick" | "candle"): "rise" | "fall" {
		if (Number(start) > Number(end)) {
	...
	const getDataSpaceFromNumberScope = function (dataScope: numberScopeString, start: number, end: number) {
	...
	const getDataY = function (dataScope: numberScopeString, dataPoint: string): number {
		let space = Number(dataScope.end) - Number(dataScope.start);
	...
	const computSingalCandledata = function (
		...
		dataitem.candleColor = getCandleColor(dataitem.open as string, dataitem.close as string, "candle");
		dataitem.wickColor = getCandleColor(dataitem.open as string, dataitem.close as string, "wick");
		dataitem.candleStateus = getCandleStatus(dataitem.open as string, dataitem.close as string, "candle");
		dataitem.wickStateus = getCandleStatus(dataitem.open as string, dataitem.close as string, "wick");
		//快速渲染
		...
		try {
			...
			//记录视图范围内最末尾的candle
			if (
				xAxis.data.lineSize.width - xAxis.data.x - dataitem.currentTick!.pixSpace!.start >= 0 &&
	 ...
	 * @returns
	 */
	const findDataByTicks = function (tickArr: tickItem[]): IcandleData[] | { data: IcandleData[]; scope: numberScopeString } {
		...
		for (let inde_i = 0; inde_i < tickArr.length; inde_i++) {
			...
			if (typeof allComputedCandleData.current[item.value] !== "undefined") {
				let dataitem: IcandleData = allComputedCandleData.current[item.value] as IcandleData;
	...
	const initDynamicData = async function () {
		 ...
		 * 获得末尾时间
		 */
		let startTime = xAxis.data.currentTimeType!.backwardTimeUnit!(endTime, preTime, baseConfig!.timeZone!.displayTimeZone!);
	...
	const lunchDynamicData = async function (endTime: number) {
		 ...
		 * 获得末尾时间
		 */
		let startTime = xAxis.data.currentTimeType!.backwardTimeUnit!(endTime, initArgs.dynamicData!.dataFetchCountPreTime!, 0);
	...
	*/
			...
			if (_displayCandleData.length > 0 && currentScopeDisplayCandleData.length > 0) {
				...
				*/
				...
				if (
					...
					xAxis.data.currentTimeType!.backwardTimeUnit!(Number(currentScopeDisplayCandleData[0].time), rangeNamber, 0)!
				) {
					let i = 0;
					let count = 0;
					let starttime = xAxis.data.currentTimeType!.backwardTimeUnit!(Number(currentScopeDisplayCandleData[0].time), rangeNamber, 0)!;
				...
				if (
					...
					xAxis.data.currentTimeType!.forwardTimeUnit!(
						Number(currentScopeDisplayCandleData[currentScopeDisplayCandleData.length - 1].time),
						rangeNamber,
						0
					)!
				) {
	...
	const reComputAllDisplayedCandleData = function () {
		...
		let scale = xAxis.data.linePosition.y / (xAxis.data.linePosition.y! * yAxis.initArgs.displayPadding! + xAxis.data.linePosition.y);
		let y = (xAxis.data.linePosition.y! * yAxis.initArgs.displayPadding!) / 2;
		settotalDataPIXHeight(xAxis.data.linePosition.y!);
		setminy(y * scale);
		setcleanY(0);
		...
		if (orgCandleData.length !== 0 && displayCandleData.length !== 0) {
			...
			if (latestCandleItem === null) {
				if (isStaticData) {
					...
					dataItem = allComputedCandleData.current[displayCandleData[displayCandleData.length - 1].time] as IcandleData;
				}
			} else {
	...
	let updateLatestdisplayLatestCandle = function () {
		if (displayLatestCandle !== null) {
			...
			let _displayLatestCandle = computSingalCandledata({ ...displayLatestCandle }, edgeScope.dataScope, true);
