...
import {
  binarySearchByKeyStrictlyEqual,
...
export const findRoundTimeCountFromArray = function(
  ...
  //查找数量
  let findRoundTimeCount = function(
    timeType: ItimeFormat
  ): { count: number; startIndex: number; step: number } {
    let result_c = { count: 0, startIndex: 0, step: 0 };
    //先取整
    let round = timeType.roundingFunction(end, getTimeShift());
    //然后从后往前找，看看有没有这个时间
    let latestEqualIndex = binarySearchByKeyStrictlyEqual(
      array,
      round,
      'forEnd',
      key
    ...
    //找到了它就根据它继续找下一个，获得跨度
    let nextRound = timeType.backwardSingleUnit(
      getItemTime(array, latestEqualIndex),
      getTimeShift()
    );
    //找到下一个这个时间类型的整数点
    let nextEqualIndex = binarySearchByKeyStrictlyEqual(
