 ...
 *@returns {number}
 */
export const getSpaceSize = function(
  arg: number | string,
  length: number
): number {
 ...
 * 求tick的交集
 */
export const findIntersection = function(
  tickArr: tickItem[],
  scope: numberScope
): tickItem[] {
  let result: tickItem[] = [];
  ...
  if (tickArr.length < 300) {
    ...
    result = (findIntersectionByKey(
      (tickArr as unknown) as jsonObjectType[],
      scope,
      'value'
 ...
 * 求candle的交集
 */
export const findIntersectionCandle = function(
  candle: IcandleData[],
  scope: numberScope
): IcandleData[] {
  ...
  result = (findIntersectionByKey(
    (candle as unknown) as jsonObjectType[],
    scope,
    'time'
...
//换算区块链的数字单位
export const shiftNumber = function(
 ...
 * “四舍五入”到最近的十的倍数 (N的倍数)
 */
export const roundToNearestTenBigNumber = function(
  num: string,
  intGetPar: number
) {
  // 创建BigNumber实例
...
export function binarySearchByKeyStrictlyEqual(
  ...
  if (
    targetType === 'forStart' &&
    target === (getItem(inputArr, 0) as number)
  ) {
    return 0;
  }
  if (
    targetType === 'forEnd' &&
    target === (getItem(inputArr, inputArr.length - 1) as number)
  ) {
    return inputArr.length - 1;
  }
...
function binarySearchByKey(
  ...
  if (
    targetType === 'forEnd' &&
    target >= (inputArr[inputArr.length - 1][key] as number)
  ) {
    return inputArr.length - 1;
  }
...
export const findIntersectionByKey = function(
  ...
  if (
    ...
    (inputArr[endIndex][key] as number) < scope.end
  ) {
    // 调整endIndex以确保包含等于scope.end的元素
    endIndex++;
  }
...
//通过语言信息获得单位换算
export const getUnitNumber = function(
  _num: number,
  _lan: string,
  _fix: number
): string {
 ...
 * @returns 返回共可取多少元素
 */
export const countSelectedElements = function(
  length: number,
  step: number
): number {
  // 计算数组长度
