 ...
 */
export const anyTimeToGMT0000ToTarget = function (time: number | Date, currentTimeZone: number | "local", targetTimeZone: number | "local") {
	let date = new Date(time);
 ...
 * @param targetType 找 起点<= 目标  还是 终点>= 目标
 */
function binarySearchByKey(inputArr: jsonObjectType[], target: number, key: string, targetType: "forStart" | "forEnd"): number | null {
 ...
 * @returns 返回找到的数组范围
 */
export const findIntersectionByKey = function (inputArr: jsonObjectType[], scope: numberScope, key: string): jsonObjectType[] {
