 ...
 * k线context
 **/
const candleViewContext = createContext<IcandleViewContext>({} as IcandleViewContext);
...
const useCandleView: TcandleViewContext = function (args): IcandleViewContext {
	 ...
	 *默认参数状态
	 */
	const [initArgs, setinitArgs] = useState<IuseCandleView>(lodash.merge(DEFAULTVALUES, args));
	 ...
	 */
	const setTimeFormat = function (
		//类型
		value: TtimeType,
		//数据所在时区
		dataSourceTimeZone?: number,
		//请求数据时将时间参数调整在哪个时区
		fetchConditionTimeZone?: number,
		//显示时调整到哪个时区
	 ...
	 * 更改语言
	 */
	const setLanguage = function (language: "zh" | "en") {
		let _initArgs = { ...initArgs };
	 ...
	 * 更改时区设定
	 */
	const setTimeZone = function (timeZone: Itimezone) {
		let _initArgs = { ...initArgs };
	...
	useEffect(
		...
		[initArgs.timeFormat, initArgs.yAxis?.labelSpace!, initArgs?.timeZone]
	);
