...
export interface IyAxisobj {
	...
	funcs: {
		...
		expandDataSpanceEdge: <T extends numberScopeString>(
			input: T
		) => {
			dataScope: T;
			currentDataSpace: string;
		};
		expandDataSpanceEdgePIX: <T extends numberScope>(
			input: T
		) => {
			dataScope: T;
			currentDataSpace: string;
 ...
 * x轴钩子定义
 */
export type TAxis = (args: Iaxis, otherAxis?: IAxisobj, totalConfig?: IuseCandleView) => IAxisobj;
