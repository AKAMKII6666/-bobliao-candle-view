 ...
 */
import React, { useEffect, useRef, useState, FC, ReactElement, createContext, useContext, MouseEvent, WheelEvent, memo, TouchEvent } from "react";
...
export const CandleView: FC<iprops> = memo(({}, _ref): ReactElement => {
	//===============useHooks=================
	...
	const onTouchStartContainer = function (event: TouchEvent<HTMLDivElement>) {
		//开启了正在加载时阻止用户操作
		if (CVData.initArgs.data?.dynamicData?.stopUserOperateWhenLoading === true && CVData.hookObjs.candleObj.data.isFetchingData) {
			return;
		}
