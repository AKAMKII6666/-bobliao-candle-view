...
const webWorker: Worker = self as unknown as Worker; // self代表子线程自身，即子线程的全局对象
webWorker.addEventListener("message", (event) => {
	//webWorker.postMessage(sum);
	let inputData = event.data;
	let _xAxisdatatickArr = [...inputData._xAxisdatatickArr];
	let _viewSize = { ...inputData._viewSize };
	let _org_displayCandleMaxMin = { ...inputData._org_displayCandleMaxMin };
	 ...
	 *@returns {number}
	 */
	const getSpaceSize = function (arg: number | string, length: number): number {
	...
	const findDataByTicks = function (tickArr: tickItem[]): IcandleData[] | { data: IcandleData[]; scope: numberScopeString; maxVolume: number } {
		...
		for (let inde_i = 0; inde_i < tickArr.length; inde_i++) {
			...
			if (typeof allComputedCandleData_current[item.value] !== "undefined") {
				let dataitem: IcandleData = allComputedCandleData_current[item.value] as IcandleData;
