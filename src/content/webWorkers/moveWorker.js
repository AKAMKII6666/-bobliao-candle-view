...
webWorker.addEventListener("message", (event) => {
	...
	for (var item of result.data) {
		...
		if (isQuickUpdateing_current) {
			if (Number((_a = item.wickPixPosition) === null || _a === void 0 ? void 0 : _a.y) + Number(item.wickLength) > orgMaxMiny.start) {
				//求（像素）y最大值
				orgMaxMiny.start = Number((_b = item.wickPixPosition) === null || _b === void 0 ? void 0 : _b.y) + Number(item.wickLength);
			}
			//求（像素）y最小值
			...
			if (
				Math.max(
					Number((_f = item.candlePixPosition) === null || _f === void 0 ? void 0 : _f.y) + Number(item.candleLength),
					Number((_g = item.wickPixPosition) === null || _g === void 0 ? void 0 : _g.y) + Number(item.wickLength)
				) > orgMaxMiny.start
			) {
				//求（像素）y最大值
				orgMaxMiny.start = Math.max(
					Number((_h = item.candlePixPosition) === null || _h === void 0 ? void 0 : _h.y) + Number(item.candleLength),
					Number((_j = item.wickPixPosition) === null || _j === void 0 ? void 0 : _j.y) + Number(item.wickLength)
				);
			}
			//求（像素）y最小值
			if (
