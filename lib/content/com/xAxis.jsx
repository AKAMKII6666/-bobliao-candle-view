...
const XAxis = ({}, _ref) => {
    ...
    const makeTicks = function () {
        ...
        for (var item of CVData.hookObjs.xAxisObj.data.displayTickArr) {
            result.push(<React.Fragment key={index + "_d"}>
					<Rectangle color={PIXI.utils.string2hex(item.color)} size={{
                    width: item.size,
                    height: item.length,
                }} position={item.cPosition} alignX="center" alignY="top"></Rectangle>
				</React.Fragment>);
            index++;
        }
        return result;
    ...
    const makeLabels = function () {
        ...
        let index = 0;
        for (var item of CVData.hookObjs.xAxisObj.data.displayTickArr) {
            result.push(<React.Fragment key={index + "_a"}>
					<Text anchor={0.5} x={item.cPosition.x} y={item.cPosition.y + CVData.hookObjs.xAxisObj.data.labelSpace / 2} text={(function () {
                ...
                })()} resolution={2} style={new PIXI.TextStyle({
                    align: "center",
    ...
    const makeTooltip = function () {
        if (!CVData.hookObjs.xAxisObj.initArgs.tooltip.enabled ||
            CVData.hookObjs.xAxisObj.data.tooltipState === null ||
            !CVData.hookObjs.xAxisObj.data.tooltipIsShow) {
            return null;
        }
        return (<>
				<DashedLine color={PIXI.utils.string2hex(CVData.hookObjs.xAxisObj.initArgs.tooltip.color)} size={CVData.hookObjs.xAxisObj.data.tooltipState.size} positionStart={CVData.hookObjs.xAxisObj.data.tooltipState.position} positionStop={{
                x: CVData.hookObjs.xAxisObj.data.tooltipState.position.x,
                y: CVData.hookObjs.xAxisObj.data.tooltipState.length,
            }} dashLength={CVData.hookObjs.xAxisObj.initArgs.tooltip.dashLength} spaceLength={CVData.hookObjs.xAxisObj.initArgs.tooltip.spaceLength}></DashedLine>

				<Rectangle color={PIXI.utils.string2hex(CVData.hookObjs.xAxisObj.initArgs.tooltip?.label?.backGroundColor)} size={{
                width: tooltipTextRef.current?.width + labelPadding,
                height: CVData.hookObjs.xAxisObj.data.labelSpace,
            }} position={(function () {
                if (CVData.hookObjs.xAxisObj.data.tooltipState.position.x -
                ...
                if (CVData.hookObjs.xAxisObj.data.tooltipState.position.x +
                    ...
                    return {
                        x: CVData.hookObjs.xAxisObj.data.lineSize.width -
                            tooltipTextRef.current?.width / 2 -
                            labelPadding / 2,
                        y: CVData.hookObjs.xAxisObj.data.tooltipState.relatedTickItem.cPosition.y,
                    };
				...
				<Rectangle color={PIXI.utils.string2hex(CVData.hookObjs.xAxisObj.initArgs.tooltip?.label?.color)} size={{
				...
				<Text ref={tooltipTextRef} anchor={0.5} x={(function () {
                if (CVData.hookObjs.xAxisObj.data.tooltipState.position.x -
                    tooltipTextRef.current?.width / 2 -
                    labelPadding / 2 <
                    0) {
                ...
                if (CVData.hookObjs.xAxisObj.data.tooltipState.position.x +
                    ...
                    labelPadding / 2 >
                    CVData.hookObjs.xAxisObj.data.lineSize.width) {
                    return (CVData.hookObjs.xAxisObj.data.lineSize.width -
                        tooltipTextRef.current?.width / 2 -
                        labelPadding / 2);
                }
                return CVData.hookObjs.xAxisObj.data.tooltipState.position.x;
            })()} y={CVData.hookObjs.xAxisObj.data.tooltipState.length + CVData.hookObjs.xAxisObj.data.labelSpace / 2} text={CVData.hookObjs.xAxisObj.initArgs.tooltip?.label?.formatter(CVData.hookObjs.xAxisObj.data.tooltipState.relatedTickItem)} resolution={2} style={new PIXI.TextStyle({
                align: "center",
    ...
    /* x轴背景 */
    let xBackground = useMemo(function () {
        return (<>
					{/* y轴背景 */}
					<Rectangle color={PIXI.utils.string2hex(CVData.hookObjs.xAxisObj.initArgs.backgroundColor)} size={{
                width: CVData.hookObjs.xAxisObj.data.viewSize.width,
                height: CVData.hookObjs.xAxisObj.data.labelSpace,
            }} position={CVData.hookObjs.xAxisObj.data.linePosition} alignX="left" alignY="top"></Rectangle>
				</>);
    }, [
    ...
    let xAsixElem = useMemo(function () {
        return (<>
					{/*  x轴 */}
					<Rectangle color={PIXI.utils.string2hex(CVData.hookObjs.xAxisObj.initArgs.lineColor)} size={{
    ...
    let ticks = useMemo(function () {
        return makeTicks();
    }, [CVData.hookObjs.xAxisObj.data.displayTickArr]);
    let labels = useMemo(function () {
        return makeLabels();
