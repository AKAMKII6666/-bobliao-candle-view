...
const NetLines = ({}, _ref) => {
    ...
    const makeNetLinesx = function () {
        ...
        for (var item of CVData.hookObjs.xAxisObj.data.netLineArr) {
            result.push(<React.Fragment key={index + "_c"}>
					<Rectangle color={PIXI.utils.string2hex(CVData.hookObjs.xAxisObj.initArgs.netLineColor)} size={{
                    width: item.size,
                    height: item.length,
    ...
    const makeNetLinesy = function () {
        ...
        for (var item of CVData.hookObjs.yAxisObj.data.netLineArr) {
            result.push(<React.Fragment key={index + "_b"}>
					<Rectangle color={PIXI.utils.string2hex(CVData.hookObjs.yAxisObj.initArgs.netLineColor)} size={{
                    width: item.length,
                    height: item.size,
...
//# sourceMappingURL=netLines.jsx.map