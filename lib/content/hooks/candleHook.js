...
import { anyTimeToGMT0000ToTarget, findIntersectionCandle, getRightDate, getSpaceSize, } from "../utils/consts";
 ...
 */
const useCandleHook = function (args, xAxis, yAxis, baseConfig) {
    xAxis = xAxis;
    yAxis = yAxis;
    const [initArgs, setinitArgs] = useState(lodash.merge(DEFAULTDATAVALUES, args));
    ...
    const [currentTimeTypeName, setcurrentTimeTypeName] = useState("");
    const [miny, setminy] = useState(0);
    const [isFetchingData, setisFetchingData] = useState(true);
    const [fetchDataStemp, setfetchDataStemp] = useState(-1);
    const [streamData, setstreamData] = useState(null);
    const [stopDynamicFetching, setstopDynamicFetching] = useState(false);
    const [lastMaxMiny, setlastMaxMiny] = useState({
        start: 0,
        end: 0,
    });
    const [workMessage, seworkMessage] = useState();
    const [LastScopeddcData, setLastScopeddcData] = useState([]);
    /**
     * 动态数据暂存处
     */
    const [TempDynamicData, setTempDynamicData] = useState(null);
    const [isFirstTimeUpdate, setisFirstTimeUpdate] = useState(false);
    /**
     * 是否已完成初始化
     */
    const [isFinishedInit, setisFinishedInit] = useState(false);
    const [isDQuickUpdateing, setisDQuickUpdateing] = useState(false);
    const [cursorCandleItem, setCursorCandleItem] = useState(null);
    const [latestCandleItem, setlatestCandleItem] = useState(null);
    /**
     * 上次更新的X轴时间戳
     */
    const [lastTimexAsixStemp, setlastTimexAsixStemp] = useState(798);
    /**
     * volume 数据图表的高度
     */
    const [volumChartPixHeight, setvolumChartPixHeight] = useState(0);
    /**
     * volume 数据图表 当前视窗区域的最大值
     */
    const [volumChartViewMax, setvolumChartViewMax] = useState(0);
    /**
     * 是否为静态数据模式
     */
     ...
     */
    const [orgCandleData, setorgCandleData] = useState([]);
    /**
     * 用于显示的candle数据(经过加工和归并之后的数据 )
     */
    const [displayCandleData, setdisplayCandleData] = useState([]);
    /**
     * 记录当前视窗内y轴数据的最大值和最小值
     */
    const [displayCandleMaxMin, setdisplayCandleMaxMin] = useState({ start: "0", end: "0" });
    const [org_displayCandleMaxMin, setorg_displayCandleMaxMin] = useState({ start: "0", end: "0" });
    ...
    const [latestCandleToolTip, setlatestCandleToolTip] = useState(null);
    const [latestVolumeToolTip, setlatestVolumeToolTip] = useState(null);
    /**
     * 视图范围内最末尾的candle
     * 最新的candle
     */
    const [displayLatestCandle, setdisplayLatestCandle] = useState(null);
    const [latestdisplayLatestCandle, setlatestdisplayLatestCandle] = useState(null);
    const [latestdisplayLatestVolume, setlatestdisplayLatestVolume] = useState(null);
    /**
     * view的全量尺寸
     */
    ...
    const [updateStamp, setupdateStamp] = useState(-1);
    const [initDyStamp, setinitDyStamp] = useState(-1);
    /**
     * ==================================Ref===============================
     */
    /**
    ...
    let isUpdateing = useRef(false);
    let isQuickUpdateing = useRef(false);
    let isEscapeItems = useRef(false);
    let mWorker = useRef();
    /**
     * ==========================静态常量==============================
     */
     ...
     */
    const determineTimeSpaceConsistent = function (data) {
        if (data.length === 1) {
            return "smaller";
        }
        let inputDataTimeSpace = getRightDate(data[1].time) - getRightDate(data[0].time);
    ...
    const mergeData = function (data) {
        ...
        let prevItem = data[data.length - 1];
        ...
        for (let i = data.length - 1; i > -1; i--) {
            ...
            if (_currentCandleStick.time === -1 || time !== _currentCandleStick.time) {
                if (time !== _currentCandleStick.time &&
                    _currentCandleStick.time !== -1 &&
                    typeof allComputedCandleData.current[_currentCandleStick.time] === "undefined") {
            ...
            if (time <
                xAxis.data.currentTimeType.roundingFunction(getRightDate(prevItem.time), baseConfig.timeZone.displayTimeZone) ||
     ...
     * @param data 输入数据
     */
    const putDataIntoAllComputedCandleData = function (data) {
        let _displayCandleMaxMin = {
            /**
             * 最低点
             * */
    ...
    const getCandleColor = function (start, end, type) {
        if (Number(start) > Number(end)) {
            if (type === "wick")
                return initArgs.candleStyles.wickFallColor;
            if (type === "candle")
                return initArgs.candleStyles.candleFallColor;
        }
        if (type === "wick")
            return initArgs.candleStyles.wickRiseColor;
        if (type === "candle")
            return initArgs.candleStyles.candleRiseColor;
        return "#fff";
    };
    const getCandleStatus = function (start, end, type) {
        if (Number(start) > Number(end)) {
            if (type === "wick")
                return "fall";
            if (type === "candle")
    ...
    const getDataSpaceFromNumberScope = function (dataScope, start, end) {
        ...
        let yHeight = xAxis.data.viewSize.height - getSpaceSize(xAxis.initArgs.labelSpace, xAxis.data.viewSize.height);
    ...
    orgScope) {
        ...
        dataitem.wickStateus = getCandleStatus(dataitem.open, dataitem.close, "wick");
        //快速渲染
        ...
        try {
            ...
            //记录视图范围内最末尾的candle
            if (xAxis.data.lineSize.width - xAxis.data.x - dataitem.currentTick.pixSpace.start >= 0 &&
                dataitem.currentTick.pixSpace.end - (xAxis.data.lineSize.width - xAxis.data.x) <=
    ...
    const findDataByTicks = function (tickArr) {
        ...
        for (let inde_i = 0; inde_i < tickArr.length; inde_i++) {
            ...
            if (typeof allComputedCandleData.current[item.value] !== "undefined") {
                let dataitem = allComputedCandleData.current[item.value];
    ...
    const seAllDataDateToUserConfigedGMT = function (data) {
    ...
    const initDynamicData = async function () {
        //清空堆数据
        Object.keys(allComputedCandleData.current).forEach((key) => delete allComputedCandleData.current[key]);
        setupdateStamp(-1);
        setorgCandleData([]);
         ...
         * 当前的整数时间
         */
        let _timeInteger = xAxis.data.currentTimeType.roundingFunction(endTime, baseConfig.timeZone.displayTimeZone);
        /**
         * 获得末尾时间
         */
        let startTime = xAxis.data.currentTimeType.backwardTimeUnit(_timeInteger, initArgs.dynamicData.dataFetchCountPreTime, baseConfig.timeZone.displayTimeZone);
        let result = await initArgs.dynamicData.dataFetchCallback(xAxis.data.currentTimeType.name, startTime, endTime);
        if (result.length !== 0) {
            setTempDynamicData(seAllDataDateToUserConfigedGMT(result));
            setisFirstTimeUpdate(true);
        }
        //计算volum图的像素高度
        let heightPixVolumArea = getSpaceSize(initArgs.candleStyles?.volumChart?.volumeChartHeight, xAxis.data.linePosition.y);
     ...
     * 在数据滚动或者缩放时发起的动态数据拉取
     */
    const lunchDynamicData = async function (endTime) {
        if (isFetchingData) {
            return;
        }
        setisFetchingData(true);
         ...
         * 获得末尾时间
         */
        let startTime = xAxis.data.currentTimeType.backwardTimeUnit(endTime, initArgs.dynamicData.dataFetchCountPreTime, baseConfig.timeZone.displayTimeZone);
        let timeZoneD = 0;
        ...
        if (result.length === 1) {
            let intTime = anyTimeToGMT0000ToTarget(Number(result[0].time), baseConfig.timeZone.dataSourceTimeZone, baseConfig.timeZone.displayTimeZone);
            //说明数据已经到头了
     ...
     * 更新动态数据
     */
    const updateDynamicData = function (data) {
        if (isFetchingData) {
            return;
        }
        if (!isFinishedInit) {
    ...
    const initStaticData = function () {
        ...
        Object.keys(allComputedCandleData.current).forEach((key) => delete allComputedCandleData.current[key]);
        //排序
        let _orgCandleData = seAllDataDateToUserConfigedGMT(initArgs.staticData.sort(function (a, b) {
            return getRightDate(a.time) - getRightDate(b.time);
        }));
        //判断时间类型是不是一致，还是大了还是小了
        ...
        let dataScope = { start: "500", end: "700" };
        //小了的话就按照配置的时间类型进行归并
        if (isConsistentOfDateType === "smaller") {
        ...
        //用于显示的数据
        let result = findDataByTicks(xAxis.data.tickArr);
    ...
    const updatePartialCandleData = function () {
        ...
        for (var item of result.data) {
            ...
            if (isQuickUpdateing_current) {
                ...
                if (Math.max(Number(item.candlePixPosition?.y) + Number(item.candleLength), Number(item.wickPixPosition?.y) + Number(item.wickLength)) > orgMaxMiny.start) {
                    //求（像素）y最大值
                    orgMaxMiny.start = Math.max(Number(item.candlePixPosition?.y) + Number(item.candleLength), Number(item.wickPixPosition?.y) + Number(item.wickLength));
                }
                //求（像素）y最小值
    ...
    */
    const updatePartialCandleDataV3 = function () {
        let onlyUpdatePositionAndScale = function (_cArr) {
            ...
            barr = _cArr;
            /* } */
            let currentScopeDisplayCandleData = findIntersectionCandle(barr, xAxis.data.currentTimeScope);
            if (isFromAppend) {
        ...
        let updateAndAppendNewCandle = function () {
            ...
            if (xAxis.data.tickArr[0].value !== _totalCandleDisplayArr[0].time) {
                let index = 0;
                while (typeof xAxis.data.tickArr[index] !== "undefined" &&
                    xAxis.data.tickArr[index].value < _totalCandleDisplayArr[0].time) {
                    let item = allComputedCandleData.current[xAxis.data.tickArr[index].value];
            ...
            let forwardDCArr = [];
            if (xAxis.data.tickArr[xAxis.data.tickArr.length - 1].value !==
                _totalCandleDisplayArr[_totalCandleDisplayArr.length - 1].time) {
                let index = xAxis.data.tickArr.length - 1;
                while (typeof xAxis.data.tickArr[index] !== "undefined" &&
                    xAxis.data.tickArr[index].value > _totalCandleDisplayArr[_totalCandleDisplayArr.length - 1].time) {
                    let item = allComputedCandleData.current[xAxis.data.tickArr[index].value];
                    if (typeof item !== "undefined") {
                        item.currentTick = xAxis.data.tickArr[index];
            ...
            let _displayCandleData = [...backwardDCArr, ..._totalCandleDisplayArr, ...forwardDCArr];
            let currentScopeDisplayCandleData = findIntersectionCandle(_displayCandleData, xAxis.data.currentTimeScope);
            let _org_displayCandleMaxMin = org_displayCandleMaxMin;
            let currentTag = _org_displayCandleMaxMin.start + _org_displayCandleMaxMin.end;
        ...
        window.requestAnimationFrame(function () {
            if (xAxis.data.tickArr.length > 120) {
                let inputArr = undefined;
                if (xAxis.data.lastTimeScope.start !== xAxis.data.currentTimeScope.start &&
    ...
    const updatePartialCandleDataWorker = function () {
        ...
        let initArgs_candleStyles_wickFallColor = initArgs.candleStyles.wickFallColor;
        let initArgs_candleStyles_candleFallColor = initArgs.candleStyles.candleFallColor;
        let initArgs_candleStyles_wickRiseColor = initArgs.candleStyles.wickRiseColor;
        let initArgs_candleStyles_candleRiseColor = initArgs.candleStyles.candleRiseColor;
        ...
        let xAxis_data_displayTickCommonpixWidth = xAxis.data.displayTickCommonpixWidth;
    ...
    const reComputAllDisplayedCandleData = function () {
        ...
        setviewSize(xAxis.data.viewSize);
        if (result.data.length !== 0) {
            let scale = xAxis.data.linePosition.y /
                (xAxis.data.linePosition.y * yAxis.initArgs.displayPadding + xAxis.data.linePosition.y);
    ...
    const checkDynamicData = async function (data) {
     ...
     * 更新最末尾的Candle
     */
    let updateLatestCandleData = function (candleItem) {
    ...
    let updateStreamData = function () {
        ...
        let { time, open, close, high, low, volume, isMergeMode } = { ...streamData };
        ...
        let currentRoundTime = xAxis.data.currentTimeType?.roundingFunction(Number(time), baseConfig.timeZone.displayTimeZone);
        ...
        if (currentRoundTime === _latestCandleItem?.time) {
            ...
            _latestCandleItem = computSingalCandledata(_latestCandleItem, org_displayCandleMaxMin, org_displayCandleMaxMin);
            //在可见范围内更新，不可见就不更新
            if (xAxis.data.currentTimeScope.start <= currentRoundTime &&
        ...
        if (!isNew) {
            ...
            if (xAxis.data.currentTimeScope.start <= currentRoundTime &&
                xAxis.data.currentTimeScope.end >= currentRoundTime) {
                xAxis.funcs.moveContainer(0, 0 -
                    (_latestCandleItem.currentTick.pixSpace.end -
                        _latestCandleItem.currentTick.pixSpace.start), true);
            }
            if (isChangeDisplayCandleArr) {
                setdisplayCandleData(_displayCandleData);
            }
        ...
        if (orgCandleData.length !== 0 && displayCandleData.length !== 0) {
            ...
            if (latestCandleItem === null) {
                if (isStaticData) {
                    ...
                    let intTime = xAxis.data.currentTimeType.roundingFunction(getRightDate(orgCandleData[orgCandleData.length - 1].time), baseConfig.timeZone.displayTimeZone);
            ...
            if (typeof dataItem !== "undefined") {
                ...
                let currentHeight = volumChartPixHeight * (Number(copyedItem.volume) / volumChartViewMax);
                let _latestVolumeToolTip = {
                    position: {
                        x: 0,
                    ...
                    relatedTickItem: null,
                    size: getSpaceSize(initArgs.candleStyles.volumChart?.currentPriceTooltip.lineSize, viewSize.width),
                };
    ...
    let updateLatestdisplayLatestCandle = function () {
        if (displayLatestCandle !== null) {
            ...
            let _displayLatestCandle = computSingalCandledata({ ...displayLatestCandle }, edgeScope.dataScope, displayCandleMaxMin);
            ...
            let currentHeight = volumChartPixHeight * (Number(_displayLatestCandle.volume) / volumChartViewMax);
            let _latestdisplayLatestVolume = {
                position: {
                    x: 0,
                    y: xAxis.data.linePosition.y - currentHeight,
                },
                ...
                relatedTickItem: null,
                size: getSpaceSize(initArgs.candleStyles.volumChart?.viewLastPriceTooltip.lineSize, viewSize.width),
            };
            setlatestdisplayLatestVolume(_latestdisplayLatestVolume);
        }
    };
    let getCurrentCursorCandle = function () {
        if (xAxis.data.tooltipIsShow &&
            ...
            let item = allComputedCandleData.current[key];
            setCursorCandleItem(item);
        }
    };
    let workerReciveMessage = function (e) {
        let data = e.data;
        ...
        if (data.message === "finishWork") {
            ...
            let expendHeight = currentheight + currentheight * yAxis.initArgs.displayPadding;
            let scale = yAxis.data.lineSize.height / expendHeight;
            let y = -data.data.orgMaxMiny.end + (currentheight * yAxis.initArgs.displayPadding) / 2;
            //
    ...
    let openMoveWorker = function () {
        ...
        mWorker.current.addEventListener("message", (e) => {
            seworkMessage(e);
        });
    };
    /**
...
//# sourceMappingURL=candleHook.js.map