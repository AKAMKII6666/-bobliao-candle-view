 ...
 * y轴钩子
 */
const useyAxis = function (args, xAxis) {
    xAxis = xAxis;
    /**
     *默认参数状态
     */
     ...
     * y轴的更新
     */
    const [yAxisUpdateTimeStamp, setyAxisUpdateTimeStamp] = useState(-1);
    /**
     * ============================tooltip state===========================
     */
    /**
     ...
     * 当前数据范围
     */
    const [currentDataScope, setcurrentDataScope] = useState({
        /**
         * 最下面的值
         */
        start: "0",
     ...
     * 当前数据整体长度（从0开始）
     */
    const [currentDataSpace, setcurrentDataSpace] = useState("0");
    /**
     * 当前数据相对0的位置(以start为基准 )
     */
    const [currentDataPositionOfStart, setcurrentDataPositionofStart] = useState("0");
    /**
     * ============================line 属性state===========================
     */
    /**
     ...
     *@returns {void}
     */
    const updateAxisSates = function (viewWidth, viewHeight, dataScope) {
    ...
    const update = function (viewWidth, viewHeight) {
        /* 设置各项属性 */
        setviewSize({
            width: viewWidth,
            height: viewHeight,
    ...
    const expandDataSpanceEdge = function (input) {
        let result = { ...input };
    ...
    const expandDataSpanceEdgePIX = function (input) {
        let result = { ...input };
     ...
     * 取得显示tick
     */
    const computDisplayTicks = function (tickSeed) {
        let result = [];
        let index = 0;
        for (var item of tickSeed) {
            result.push({
                ...
                cPosition: {
                    x: linePosition.x,
                    y: (function () {
                        let pre = Number(new _bigNumber(item)
     ...
     * 取得显示line
     */
    const computDisplayLines = function (tickSeed) {
        let result = [];
        let index = 0;
        for (var item of tickSeed) {
            result.push({
...
//# sourceMappingURL=yAxisHook.js.map