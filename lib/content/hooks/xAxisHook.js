 ...
 * x轴钩子
 */
const usexAxis = function (args, igorn, config) {
    const moveThrettor = useThrottle();
    /**
     *默认参数状态
     */
    const [initArgs, setinitArgs] = useState(Object.assign(true, DEFAULTAXISVALUES, args));
    const [initArgsChange, setinitArgsChange] = useState(-1);
    //鼠标移动速度检测
    const [mousePosition, setmousePosition] = useState(0);
    const [lastmousePosition, setlastmousePosition] = useState(0);
    const [mouseSpeedSec, setmouseSpeedSec] = useState(0);
    const [mouseSpeedTemp, setmouseSpeedTemp] = useState(0);
    const [mouseSpeedTimmer, setmouseSpeedTimmer] = useState();
    /**
     * ============================state===========================
     */
     ...
     * 普通的更新状态（位移，缩放）
     */
    const [xAxisUpdateTimeStamp, setxAxisUpdateTimeStamp] = useState(-1);
    const [xAxisUpdateMoveMentTimeStamp, setxAxisUpdateMoveMentTimeStamp] = useState(-1);
    const [xAxisUpdateScaleTimeStamp, setxAxisUpdateScaleTimeStamp] = useState(-1);
    /**
     * ============================tooltip state===========================
     */
    /**
    ...
    const [orgScope, setorgScope] = useState({ start: 0, end: 0 });
    /**
     * 每次缩放的增减值
     */
     ...
     * y轴的label空间
     */
    const [yAxisSpace, setyAxisSpace] = useState(0);
    /**
     * line的位置（上左定位）
     *内容区和label区的分割线的位置
     */
     ...
     * 网格线组
     */
    const [netLineArr, setnetLineArr] = useState([]);
    /**
     * 网格线的最大数量
     */
    const [netLineMaxCount, setnetLineMaxCount] = useState(0);
    /**
     * 轴网格线最小数量
     */
    const [netLineMinCount, setnetLineMinCount] = useState(0);
    /**
     * ============================tick 属性state===========================
     */
    /**
     * 真实tick组（实际的tick组）
     */
    const [tickArr, settickArr] = useState([]);
    /**
     * 显示tick组（用于显示的tick）
     */
    const [displayTickArr, setdisplayTickArr] = useState([]);
    /**
     * tick的共有数据宽度
     */
    const [displayTickCommonWidth, setdisplayTickCommonWidth] = useState(0);
    /**
     * tick的共有像素宽度
     */
    const [displayTickCommonpixWidth, setdisplayTickCommonpixWidth] = useState(0);
    /**
     * ============================ref===========================
     */
    const [candleObj, setcandleObj] = useState(null);
    /**
     * ============================静态变量===========================
     */
    /**
     ...
     *@returns {void}
     */
    const initAxisSates = function (timeType, viewWidth, viewHeight, yAxisLabelSpace) {
        /* 设置各项属性 */
        setviewSize({
            width: viewWidth,
            height: viewHeight,
     ...
     * 往以前推测时间
     */
    const timeSpeculation_backrward = function (timeInteger, initTimeScopeStart) {
     ...
     * 往未来推测时间
     */
    const timeSpeculation_forward = function (timeInteger, initTimeScopeEnd) {
        var currentTime = timeInteger;
        let timeScopeArr = [timeInteger];
        while (true) {
     ...
     * 扩展tick组
     */
    const updateTicks = function (targetTickArr, timeScope, isComputCommonProp, _moveAmount, moveDir) {
        if (moveDir === "add" || moveDir === "all") {
            let forwardArr = timeSpeculation_forward(targetTickArr[targetTickArr.length - 1].value, timeScope.end);
        ...
        if (moveDir === "min" || moveDir === "all") {
            let backwardArr = timeSpeculation_backrward(targetTickArr[0].value, timeScope.start);
        ...
        if (isComputCommonProp) {
            ...
            for (var item of targetTickArr) {
                ...
                //计算位置
                item.cPosition = {
                    x: getRangePosition(Number(item.value), timeScope, width) -
     ...
     * 扩展displaytick组
     */
    const updateDisplayTicks = function (targetTickArr, newTickArr) {
    ...
    //计算tick的共有属性
    const computTickCommonProp = function (range, width, totalArrLength) {
     ...
     * 计算tick的位置和数据关系数据
     */
    const computDataPixTick = function (item, range, index, dataWidth, pixWidth) {
        item.dataSpace = {
     ...
     * 创建真实tick组
     */
    const createTickers = function (arr, range, isComputCommonProp, moveAmount) {
        let result = [];
        let commonPixProperties;
        if (isComputCommonProp) {
        ...
        for (var item of arr) {
            ...
            resultItem.cPosition = {
                x: getRangePosition(Number(resultItem.value), range, width) -
                    commonPixProperties.incriseWidth -
                    moveAmount,
                y: linePosition.y,
            };
     ...
     */
    const createDisplayTickers = function (arr) {
        let result = arr;
        let _tickStep = 0;
        while (true) {
    ...
    //};
    //ai 优化后的版本
    const findTick = function (position, key) {
     ...
     * 创建x轴的网格 (纵向)
     */
    const createNetLines = function (displaytickItems) {
        let results = [];
    ...
    //2.合并两个数组
    //3.输出
    const createDisplayTickersByDate = function (tickArr, displayTickRoundValuesArray) {
        tickArr = [...tickArr];
        let displayTickArr1 = [];
        let displayTickArr2 = [];
        let isFind = false;
    ...
    const getTickWithFormated = function (tickArr, xCondition) {
        let result = [];
        //先取到第一个
    ...
    const createAxisData = function () {
        ...
        */
        ...
        let displayTickRoundValuesArray = findRoundTimeCountFromArray(_tickerArr, config.timeZone.displayTimeZone, config.timeFormat, "value");
     ...
     * 移动轴(鼠标拖拽)
     */
    const moveContainer = function (start, stop, isSaveScope) {
        if (isFinishedInit) {
     ...
     * 移动轴(鼠标拖拽)
     */
    const moveAxis = function (start, stop, isSaveScope) {
        ...
        if (isFinishedInit) {
             ...
             * 用剩下的tick去进行扩展
             */
            let newTicks = updateTicks(resultInterArr, _currentTimeScope, false, _moveAmount, (function () {
            ...
            //挑选出所有按时间整数排列的等差数列的参数
            let displayTickRoundValuesArray = findRoundTimeCountFromArray(newTicks, config.timeZone.displayTimeZone, config.timeFormat, "value");
            let _displayTickerArr;
            if (displayTickRoundValuesArray === null) {
                /**
                 * 求新时间范围和旧显示tick的交集
     ...
     * 缩放
     */
    const scale = function (point, precent, movement) {
        if (candleObj?.data.isFetchingData && config?.data?.dynamicData?.stopUserOperateWhenLoading) {
            return;
        }
    ...
    const tooltipUpdate = function () {
        if (isFinishedInit && tooltipIsShow) {
            let _tooltipState = tooltipState;
     ...
     * tooltip移动
     */
    const tooltipMove = function (position, isShowTooltip) {
        if (isFinishedInit) {
            ...
            //通过像素位置进行二分法查找目标tick
            let _tickItem = findTick(tooltipX - moveAmount, "pixSpace");
            if (_tickItem === null) {
     ...
     *@returns {void}
     */
    const updateAxisSates = function (viewWidth, viewHeight, yAxisLabelSpace) {
        setviewSize({
            width: viewWidth,
            height: viewHeight,
        });
...
//# sourceMappingURL=xAxisHook.js.map