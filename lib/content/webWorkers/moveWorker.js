...
webWorker.addEventListener("message", (event) => {
    ...
    const getCandleColor = function (start, end, type) {
        if (Number(start) > Number(end)) {
            if (type === "wick")
                return initArgs_candleStyles_wickFallColor;
            if (type === "candle")
                return initArgs_candleStyles_candleFallColor;
        }
        if (type === "wick")
            return initArgs_candleStyles_wickRiseColor;
        if (type === "candle")
            return initArgs_candleStyles_candleRiseColor;
        return "#fff";
    };
    ...
    orgScope) {
        try {
            ...
            //记录视图范围内最末尾的candle
            if (xAxis_data_lineSize_width - xAxis_data_x - dataitem.currentTick.pixSpace.start >= 0 &&
...
export {};
//# sourceMappingURL=moveWorker.js.map