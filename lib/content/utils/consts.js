 ...
 */
export const localTimeToGMT0000 = function (time) {
    let date = new Date(time);
    let timeZone = Math.abs(date.getTimezoneOffset() / 60);
    date.setHours(date.getHours() - timeZone);
    return date.getTime();
};
export const GMT0000TolocalTime = function (time) {
    let date = new Date(time);
    let timeZone = Math.abs(date.getTimezoneOffset() / 60);
    date.setHours(date.getHours() + timeZone);
    return date.getTime();
 ...
 */
export const anyTimeToGMT0000ToTarget = function (time, currentTimeZone, targetTimeZone) {
    let date = new Date(time);
 ...
 *@returns {number}
 */
export const getSpaceSize = function (arg, length) {
 ...
 * 求tick的交集
 */
export const findIntersection = function (tickArr, scope) {
    let result = [];
 ...
 * 求candle的交集
 */
export const findIntersectionCandle = function (candle, scope) {
    let result = [];
...
//换算区块链的数字单位
export const shiftNumber = function (_number, _shiftLength) {
    return new _bigNumber(_number).times(new _bigNumber(10).exponentiatedBy(_shiftLength)).toString();
};
/**
 * 将数组快速转换为hash表
 */
export const arrayToHash = function (arr, keyProperty) {
 ...
 * 获得某个数字的小数位数
 */
export const getDecimalOfLength = function (num) {
    return new _bigNumber(num).decimalPlaces();
};
/**
 * 获得某个数字的整数位数
 */
export const getIntOfLength = function (num) {
 ...
 * “四舍五入”到最近的十的倍数 (N的倍数)
 */
export const roundToNearestTenBigNumber = function (num, intGetPar) {
    // 创建BigNumber实例
 ...
 * @param targetType 找 起点=== 目标  还是 终点=== 目标
 */
export function binarySearchByKeyStrictlyEqual(inputArr, target, targetType, key) {
    let getItem = function (arr, index) {
 ...
 * @returns 返回找到的数组范围
 */
export const findIntersectionByKey = function (inputArr, scope, key) {
    let startIndex = binarySearchByKey(inputArr, scope.start, key, "forStart");
    let endIndex = binarySearchByKey(inputArr, scope.end, key, "forEnd");
    // 确保索引有效
    startIndex = startIndex === null ? 0 : startIndex;
 ...
 * 获得正确的时间
 */
export const getRightDate = function (dateTime) {
...
//千分位分割
export const thousandsSplit = function (num) {
...
//通过语言信息获得单位换算
export const getUnitNumber = function (_num, _lan, _fix) {
...
//韩文
export const translateNumberK = function (_num, _fix) {
    if (typeof _fix === "undefined") {
        _fix = 0;
    }
...
//中文
export const translateNumberF = function (_num, _fix) {
    if (typeof _fix === "undefined") {
        _fix = 0;
    }
    var num = new _bigNumber(_num).toFixed().split(".");
...
//换算单位英文
export const translateNumberT = function (_num, _fix) {
    if (typeof _fix === "undefined") {
        _fix = 0;
    }
    var num = new _bigNumber(_num).toFixed().split(".");
 ...
 * @returns 返回共可取多少元素
 */
export const countSelectedElements = function (length, step) {
    // 计算数组长度
 ...
 * 检查数组里的元素是否有重复
 */
export const hasDuplicates = function (arr) {
...
//获得两点之间的距离
export const getLength = function (p1, p2) {
